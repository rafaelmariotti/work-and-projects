...
function run_dump {
	...
	pg_dump -d ${database} -h ${hostname} -U ${username} -f ${dump_home}/${database}/${dump_dir}/${database}.dmp > /tmp/pg_dump_${database}.log
	...
	if [ $(wc -l /tmp/pg_dump_${database}.log | awk '{print $1}') -gt 0 ]; then
		...
		mail -r report@bionexo.com -s "pg_dump ${database} - error [$(date +"%d/%m/%Y %H:%M")]" -a /tmp/pg_dump_${database}.log dba@bionexo.com < /tmp/mail_body.txt
